...
const fmtWeekLabel = (s,lang) => { const a=new Date(s+"T12:00:00"),b=new Date(s+"T12:00:00"); b.setDate(b.getDate()+6); const M=MONTHS_SHORT[lang||"es"]; return `${a.getDate()} ${M[a.getMonth()]} – ${b.getDate()} ${M[b.getMonth()]}`; };
...
const Toggle = ({value,onChange,label,desc,T,sc}) => (
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:`1px solid ${T.border}`}}>
    <div><div style={{fontSize:fs(13,sc),fontWeight:600,color:T.text}}>{label}</div>{desc&&<div style={{fontSize:fs(11,sc),color:T.textMute,marginTop:2}}>{desc}</div>}</div>
...
const CardChip = ({card,selected,onClick,T,sc}) => (
  <button onClick={onClick} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:selected?`${card.color}22`:T.surface,border:`1.5px solid ${selected?card.color:T.border}`,borderRadius:10,cursor:"pointer",textAlign:"left",width:"100%",marginBottom:7}}>
    ...
    <div style={{flex:1,minWidth:0}}><div style={{fontSize:fs(13,sc),fontWeight:700,color:selected?card.color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{card.name}</div><div style={{fontSize:fs(10,sc),color:T.textMute}}>**** {card.last4||"****"}</div></div>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:8}}>
        {AVATARS.map(av=>(
          <button key={av.id} onClick={()=>onChange(av.id)} style={{padding:0,background:"none",border:`2.5px solid ${current===av.id?"#3b82f6":"transparent"}`,borderRadius:10,cursor:"pointer",transition:"border 0.15s"}}>
      ...
      <div style={{background:T.header,borderRadius:"20px 20px 0 0",padding:"20px 18px 40px",width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginTop:22,background:isDarkBg(T.bg)?"#0d2137":"#eff6ff",border:"1px solid #2563eb33",borderRadius:12,padding:"14px"}}>
          ...
          <div style={{fontSize:fs(11,sc),color:T.textMute,lineHeight:1.7}}>
            {TR.privacyLines[lang||"es"].map((l,i)=><div key={i}>• {l}</div>)}
          </div>
        </div>
      </div>
    </div>
...
function AuthScreen({T,isDark,toggleTheme,sc,lang,setLang}) {
  ...
  const errMap=(code)=>({
    "auth/email-already-in-use":t("errEmailUsed",lang),
    "auth/invalid-credential":t("errCreds",lang),
    "auth/wrong-password":t("errCreds",lang),
    "auth/user-not-found":t("errCreds",lang),
    "auth/invalid-email":t("errInvalidEmail",lang),
    "auth/weak-password":t("errWeakPass",lang),
  ...
  const handleSubmit=async()=>{
    ...
    try {
      if(mode==="register"){
        ...
        // Store username mapping + profile
    ...
    <div style={{background:T.bg,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px"}}>
      ...
      <div style={{width:"100%",maxWidth:400,background:T.surface,borderRadius:20,padding:"26px 22px",border:`1px solid ${T.border}`,boxShadow:isDark?"0 8px 40px #00000060":"0 8px 40px #00000015"}}>
        ...
        {mode==="register"&&<>
          ...
          <input value={username} onChange={e=>setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/,""))} placeholder={t("username",lang)} style={{...inp,marginBottom:4}}/>
        ...
        {showPrivacy&&(
          <div style={{marginTop:8,background:isDark?"#0d2137":"#eff6ff",border:"1px solid #2563eb33",borderRadius:10,padding:"12px 14px",fontSize:fs(11,sc),color:T.textMute,lineHeight:1.6,whiteSpace:"pre-line"}}>
            {t("privacyBlock",lang)}
          </div>
        )}
      </div>
...
function IncomeDashboard({expenses,incomes,currentWeek,T,sc,isDark,lang}) {
  ...
  const balance=weekInc-weekExp, savingsRate=weekInc>0?((balance/weekInc)*100):0;
  const now=new Date(), M=MONTHS_SHORT[lang]||MONTHS_SHORT.es;
  ...
  const incLabel=lang==="zh"?"收入":lang==="en"?"Income":"Ingresos", expLabel=lang==="zh"?"支出":lang==="en"?"Expenses":"Gastos";
  return (
    <div>
        ...
        <div key={title} style={{background:T.surface,borderRadius:14,padding:14,marginBottom:16,border:`1px solid ${T.border}`}}>
          ...
          <ResponsiveContainer width="100%" height={155}>
            <BarChart data={data} barSize={13} barGap={2}>
              ...
              <YAxis hide/><Tooltip contentStyle={tooltipStyle} formatter={(v,n)=>[fmtMXN(v),n==="ingresos"?incLabel:expLabel]} cursor={{fill:"#00000008"}}/>
              <Bar dataKey="ingresos" fill="#22c55e" radius={[4,4,0,0]}/><Bar dataKey="gastos" fill="#ef4444" radius={[4,4,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
      ...
      {monthsData.some(m=>m.balance!==0)&&(
        <div style={{background:T.surface,borderRadius:14,padding:14,marginBottom:16,border:`1px solid ${T.border}`}}>
          ...
          <ResponsiveContainer width="100%" height={115}>
            <LineChart data={monthsData}><XAxis dataKey="label" tick={{fontSize:fs(9,sc),fill:T.textMute}} axisLine={false} tickLine={false}/><YAxis hide/><Tooltip contentStyle={tooltipStyle} formatter={(v)=>[fmtMXN(v),"Balance"]} cursor={{stroke:T.border}}/><Line type="monotone" dataKey="balance" stroke="#a855f7" strokeWidth={2} dot={{fill:"#a855f7",r:3}} activeDot={{r:5}}/></LineChart>
          </ResponsiveContainer>
        </div>
      )}
      {incBycat.length>0&&(
    ...
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {[["MXN","🇲🇽"],["USD","🇺🇸"]].map(([cur,flag])=>(<button key={cur} onClick={()=>setCurrency(cur)} style={{background:currency===cur?"#22c55e":T.surface,border:`1.5px solid ${currency===cur?"#22c55e":T.border}`,borderRadius:8,padding:"6px 10px",cursor:"pointer",color:currency===cur?"#fff":T.textMute,fontSize:fs(13,sc),fontWeight:700}}>{flag} {cur}</button>))}
        </div>
      </div>
      ...
      <button onClick={handleSave} disabled={saving} style={{width:"100%",background:"linear-gradient(135deg,#16a34a,#15803d)",border:"none",color:"#fff",borderRadius:12,padding:"14px",fontSize:fs(16,sc),fontWeight:800,cursor:"pointer",opacity:saving?0.7:1,boxShadow:"0 4px 24px #16a34a40",marginBottom:10}}>
        {saving?t("saving",lang):t("addIncome",lang)}
      </button>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    if(!user){setExpenses([]);setIncomes([]);setCards([]);setWeekBudget(0);setProfile(null);return;}
    const uid=user.uid;
  ...
  const analyticsData=(()=>{
    ...
    const wwd=weeks.filter(w=>w.total>0); const now=new Date();
    const mS=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-01`, mE=new Date(now.getFullYear(),now.getMonth()+1,0).toISOString().slice(0,10);
    ...
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:T.bg,minHeight:"100vh",color:T.text,maxWidth:480,margin:"0 auto",paddingBottom:90,transition:"background 0.3s",fontSize:fs(14,sc)}}>
      ...
      <div style={{background:T.header,padding:"14px 16px",borderBottom:`1px solid ${T.border}`,boxShadow:isDark?"none":"0 1px 8px #00000010"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:(view==="analysis"||view==="incomes")?0:10}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <div onClick={()=>setShowProfile(p=>!p)} style={{cursor:"pointer"}}>
              <AvatarBubble avatarId={profile?.avatarId||"a1"} size={32} fontSize={16}/>
            </div>
          </div>
        </div>
        ...
        {view!=="analysis"&&view!=="incomes"&&weekBudget>0&&(
          <div style={{marginTop:8}}>
            ...
            <div style={{background:T.barEmpty,borderRadius:99,height:8,overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",borderRadius:99,transition:"width 0.5s",background:pct<60?"linear-gradient(90deg,#22c55e,#16a34a)":pct<85?"linear-gradient(90deg,#eab308,#ca8a04)":"linear-gradient(90deg,#ef4444,#dc2626)"}}/></div>
      ...
      <div style={{display:"flex",background:T.header,borderBottom:`1px solid ${T.border}`,overflowX:"auto"}}>
        {navTabs.map(([v,ico,label])=>(
          <button key={v} onClick={()=>setView(v)} style={{flex:1,minWidth:58,background:"none",border:"none",padding:"9px 4px",color:view===v?"#3b82f6":T.textMute,fontSize:fs(11,sc),fontWeight:view===v?700:400,cursor:"pointer",borderBottom:view===v?"2px solid #3b82f6":"2px solid transparent",whiteSpace:"nowrap"}}>
      ...
      <div style={{padding:"16px"}}>
        ...
        {view==="dashboard"&&(filtered.length===0?(
          <div style={{textAlign:"center",padding:"50px 0",color:T.textMute}}><div style={{fontSize:44}}>📭</div><div style={{marginTop:12,fontSize:fs(15,sc)}}>{t("noExpenses",lang)}</div><div style={{marginTop:6,fontSize:fs(13,sc)}}>{t("tapAdd",lang)}</div></div>
        ):(
          <>
            <div style={{background:T.surface,borderRadius:14,padding:14,marginBottom:16,border:`1px solid ${T.border}`}}>
              <div style={{fontSize:fs(11,sc),letterSpacing:2,color:T.textMute,textTransform:"uppercase",marginBottom:10}}>{t("expByDay",lang)}</div>
              <ResponsiveContainer width="100%" height={120}><BarChart data={dailyData} barSize={22}><XAxis dataKey="day" tick={{fontSize:fs(11,sc),fill:T.textMute}} axisLine={false} tickLine={false}/><YAxis hide/><Tooltip contentStyle={tooltipStyle} formatter={(v)=>[fmtMXN(v),t("spent",lang)]} cursor={{fill:"#00000008"}}/><Bar dataKey="total" radius={[5,5,0,0]}>{dailyData.map((d,i)=><Cell key={i} fill={d.date===todayStr()?"#2563eb":d.total>0?isDark?"#2563eb77":"#93c5fd":T.barEmpty}/>)}</Bar></BarChart></ResponsiveContainer>
            </div>
            <div style={{marginBottom:16}}>
              ...
              {byCategory.sort((a,b)=>(b.total+b.totalUSD)-(a.total+a.totalUSD)).map(c=>(<div key={c.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:9}}><span style={{fontSize:fs(17,sc),width:26,textAlign:"center"}}>{c.icon}</span><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:fs(13,sc),fontWeight:600,color:T.text}}>{catLabel(c,lang)}</span><span style={{fontSize:fs(12,sc),fontWeight:700,color:c.color}}>{c.total>0&&fmtMXN(c.total)}{c.total>0&&c.totalUSD>0&&" + "}{c.totalUSD>0&&fmtUSD(c.totalUSD)}</span></div><div style={{background:T.barEmpty,borderRadius:99,height:5}}><div style={{width:`${totalMXN>0?(c.total/totalMXN)*100:0}%`,height:"100%",borderRadius:99,background:c.color}}/></div></div></div>))}
            </div>
            {byCategory.filter(c=>c.total>0).length>0&&(
              <div style={{background:T.surface,borderRadius:14,padding:14,marginBottom:16,border:`1px solid ${T.border}`}}>
                ...
                <ResponsiveContainer width="100%" height={140}><PieChart><Pie data={byCategory.filter(c=>c.total>0)} dataKey="total" cx="50%" cy="50%" outerRadius={58} stroke={T.bg} strokeWidth={2}>{byCategory.filter(c=>c.total>0).map(c=><Cell key={c.id} fill={c.color}/>)}</Pie><Tooltip contentStyle={tooltipStyle} formatter={(v)=>[fmtMXN(v)]}/></PieChart></ResponsiveContainer>
                <div style={{display:"flex",flexWrap:"wrap",gap:"4px 10px",justifyContent:"center"}}>{byCategory.filter(c=>c.total>0).map(c=>(<div key={c.id} style={{display:"flex",alignItems:"center",gap:4,fontSize:fs(11,sc)}}><div style={{width:7,height:7,borderRadius:2,background:c.color}}/><span style={{color:T.textMute}}>{catLabel(c,lang)}</span></div>))}</div>
              </div>
            )}
            <div style={{background:T.surface,borderRadius:14,padding:14,border:`1px solid ${T.border}`}}>
        ...
        {view==="analysis"&&(expenses.length===0?<div style={{textAlign:"center",padding:"50px 0",color:T.textMute}}><div style={{fontSize:44}}>📊</div><div style={{marginTop:12,fontSize:fs(15,sc)}}>{t("noData",lang)}</div></div>:(
          <>
            ...
            <div style={{background:T.surface,borderRadius:14,padding:14,marginBottom:16,border:`1px solid ${T.border}`}}>
              ...
              <ResponsiveContainer width="100%" height={130}><BarChart data={analyticsData.chartWeeks} barSize={28}><XAxis dataKey="label" tick={{fontSize:fs(10,sc),fill:T.textMute}} axisLine={false} tickLine={false}/><YAxis hide/><Tooltip contentStyle={tooltipStyle} formatter={(v)=>[fmtMXN(v),t("spent",lang)]} cursor={{fill:"#00000008"}}/><Bar dataKey="total" radius={[5,5,0,0]}>{analyticsData.chartWeeks.map((w,i)=><Cell key={i} fill={w.isCurrent?"#2563eb":w.total>0?isDark?"#2563eb66":"#93c5fd":T.barEmpty}/>)}</Bar></BarChart></ResponsiveContainer>
            </div>
            <div style={{background:T.surface,borderRadius:14,padding:14,border:`1px solid ${T.border}`}}>
        ...
        {view==="addIncome"&&(
          <div>
            ...
            <AddIncome onSave={addIncome} onCancel={()=>setView("dashboard")} T={T} sc={sc} lang={lang}/>
          </div>
        )}
